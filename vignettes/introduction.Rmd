...
data_table = read.csv(file = "./inst/extdata/GSE13015_GPL6106_data_matrix.csv",stringsAsFactors = F)
rownames(data_table) = data_table$Symbol
data_table$Symbol = NULL

##quantile normalization using function "normalize.quantiles"" from library(preprocessCore)
data.matrix.nor <- normalize.quantiles(as.matrix(data_table))
colnames(data.matrix.nor) = colnames(data_table)
rownames(data.matrix.nor) = rownames(data_table)

data.matrix = data.matrix.nor

#Load sample annotation file
sample.info = read.csv(file = "./inst/extdata/GSE13015_GPL6106sample_info.csv",stringsAsFactors = F)
...
fingerprintplot(Individual_df, cutoff= 15)
